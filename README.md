...
| **SP** | **栈寄存器**，维护一个栈，需要两个寄存器，一个保存栈的基地址，也是**栈顶地址**，一个保存栈的偏移地址，即**栈底地址**，**在 arm64 中用 X29(FP) 寄存器保存栈的基地址，SP 寄存器 保存栈顶地址，它是可以移动的，所以我们看到函数的调用时候，通常第一句 都是 sub	sp, sp, #immediate，开辟栈空间，函数调用结束，add	sp, sp, #immediate，回复栈空间，这也是为什么，栈内存能够自动回收的原因** |
